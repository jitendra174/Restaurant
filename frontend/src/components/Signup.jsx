...
function Signup() {
    ...
    const submitLogin = (e) => {
        ...
        if (Object.keys(error).length === 0) {
            setloader(true)
            axios.post("http://localhost:5000/api/login", { Email1, Password1 }, { withCredentials: true })
                .then((res) => {
    ...
    const submitSignup = (e) => {
        ...
        if (Object.keys(Error).length === 0) {
            setloader(true)
            axios.post("http://localhost:5000/api/signup", { Name, Email, Password }, { withCredentials: true })
                .then((res) => {
                    if (res.data.status === "success") {
            ...
            <div className="sigup-section">
                <div className="signup-layout">
                    ...
                    <form className={`signup-form ${show ? "signup-hide" : "signup-show"}`} onSubmit={submitSignup} noValidate>
                        ...
                        <center>
                            <button className='signup-btn'>
                                {loader ? <RotatingLines visible={true} height="10" width="20" strokeWidth="5" animationDuration="0.75" ariaLabel="rotating-lines-loading" strokeColor='white' /> : "Signup"}
                            </button>
                        </center>
                    ...
                    <form className={`login-form  ${show ? "login-show" : "login-hide"}`} onSubmit={submitLogin} noValidate>
                        ...
                        <center>
                            <button className='signup-btn'>
                                {loader ? <RotatingLines visible={true} height="25" width="20" strokeWidth="5" animationDuration="0.75" ariaLabel="rotating-lines-loading" strokeColor='white' /> : "Login"}
                            </button>
                        </center>
